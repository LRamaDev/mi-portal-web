(function (root, factory) {
  ...
  function getJSON(path) { var url=DATA_BASE+path;return root.fetch(url,{cache:'no-store'}).then(function(response){if(!response.ok)throw new Error('No se pudo abrir '+url);return response.json();}); }
  ...
  function printHeader(title,subtitle,count){return '<header class="print-head"><img class="print-logo" src="../app-transporte/assets/logo-ersep.png" alt="ERSeP"><div><h1>'+esc(title)+'</h1><p><strong>'+esc(subtitle)+'</strong></p><p>'+count.toLocaleString('es-AR')+' registros incluidos</p></div></header>';}
