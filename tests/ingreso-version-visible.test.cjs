...
test('la versión visible oficial es 6.12', () => {
  assert.match(index, /<meta name="app-version" content="6\.12">/);
  assert.match(index, />Versión 6\.12<\/div>/);
});
...
test('los recursos críticos usan URL versionada y el service worker prioriza la red', () => {
  ...
  for (const asset of ['app.js','testeo.js','ui-v6-4.js','adaptive-v6-8.js','choice-order-v6-8-2.js','badges-v6-9.js']) {
    assert.ok(index.includes(`./${asset}?v=6.12`), `${asset}: debe usar URL versionada`);
  }
  assert.match(app, /serviceWorker\.register\('\.\/sw\.js\?v=6\.12'\)/);
